...
RPA.prototype = {
    ...
    bind: function bind() {
        ...
        _this.config.drawerPanel.obj.bind('click', function (e) {
            var d = $(this).parents('.drawerPanel-container');
            if (d.hasClass('show')) {
                d.removeClass('show');
            } else {
                d.addClass('show');
    ...
    tags: {
        ...
        addTags: function addTags(_this, e) {
            var obj = $(e.target);
            var href = obj.attr('href') ? obj.attr('href') : obj.parents('a').attr('href');
            var tagsList = void 0,
                html = void 0;
            if ('#' != href) {
                var data = {
                ...
                var array = _defineProperty({}, obj[0].innerText, data);
                if (tagsList = localStorage.getItem('tagsList')) {
                    tagsList = JSON.parse(tagsList);
        ...
        setHtml: function setHtml(_this, tagsList) {
            var html = '';
            for (item in tagsList) {
                if (tagsList[item].type) {
                    html += "首页" == item ? '<a href="' + tagsList[item].uri + '"><span class="tags-item active">' + tagsList[item].title + ' </span></a>' : '<a href="' + tagsList[item].uri + '"><span class="tags-item active">' + tagsList[item].title + ' <span class="fa fa-remove tags-close" onclick="RPA.tags.delTags(event);"></span></span></a>';
                } else {
                    html += "首页" == item ? '<a href="' + tagsList[item].uri + '"><span class="tags-item">' + tagsList[item].title + ' </span></a>' : '<a href="' + tagsList[item].uri + '"><span class="tags-item">' + tagsList[item].title + ' <span class="fa fa-remove tags-close" onclick="RPA.tags.delTags(event);"></span></span></a>';
                }
            }
    ...
    screenOperation: {
        ...
        isFullscreenForNoScroll: function isFullscreenForNoScroll() {
    ...
    pjaxOperation: {
        init: function init() {
            ...
            e.obj.on({
                'pjax:timeout': function pjaxTimeout(event) {
            ...
            $(document).on('click', e._search, function (event) {
                var v = $(s + ' input').val();
                var url = $(s + ' datalist option[value="' + v + '"]').data('href');
    ...
    modalMove: function modalMove(_this) {
        ...
        $(document).ready(function () {
            ...
            $(document).on("mousemove", function (e) {
                ...
                var width = moveTarget.width();
                var height = moveTarget.height();
    ...
    initList: function initList(e) {
        //缓存菜单
        var sidebarList = localStorage.getItem("sidebarList");
        if (!sidebarList) {
            var data = [];
            $(this.config.search._sidebar).each(function (index) {
                if (!$(this).hasClass('has-treeview')) {
                    var _i = [$(this).find('a').attr('href'), $(this).find('p').text()];
                    data.push(_i);
                }
            });
            sidebarList = data;
    ...
    form: {
        reset: function reset(e, callback) {
            //重置复选框
            var formContinue = $(RPA.config.modal + ' input.icheck').each(function (e) {
                $(this).iCheck('uncheck');
            });
            $(e)[0].reset(); //重置表单，必须放下面
        },
        response: function response(callback) {
            var obj = RPA.config.modal;
            toastr.success('操作成功！');
        ...
        ajaxSubmit: function ajaxSubmit(e, FormOptions) {
        ...
        formValidation: function formValidation(arr, $form, options) {
    ...
    getIdSelections: function getIdSelections(table) {
    ...
    Echo: {
        ...
        content: function content(obj) {
            ...
            var html = "";
            html += '<div class="notify-wrap">' + '<div class="notify-title">' + typeName + '<span class="notify-off"><i class="fa fa-envelope-o"></i></span></div>' + '<div class="notify-title"><a href="JavaScript:void(0);" url="/admin/sys_message_list/view/' + obj.id + '" onclick="operation($(this));" title="查看站内信息">' + obj.title + '</a><div>' + '<div class="notify-content">' + obj.content + '</div>' + '</div>';
            ...
            if ($("#notification_count span").length > 0) {
                ...
                var html1 = '<li><a href="javascript:void(0);" onclick="operation($(this));" url="/admin/sys_message_list/view/' + obj.id + '"><i class="fa fa-users text-aqua"></i>' + obj.title + '</a></li>';
                $("#notification_list").prepend(html1);
            } else {
                $("#notification_count").append('<span class="badge badge-warning navbar-badge">1</span>');
                var _html = '<ul class="menu" id="notification_list"><li><a href="javascript:;" onclick="operation($(this));" url="/admin/sys_message_list/view/' + obj.id + '"><i class="fa fa-users text-aqua"></i>' + obj.title + '</a></li></ul>';
                $('.notifications-menu').html(_html);
            }

            var lastNotify = $(".notify-wrap:last");
    ...
    Notify: {
        popNotice: function popNotice(title, content) {
...
var operation = function operation(e) {
...
var urlEncode = function urlEncode(param, key, encode) {
    ...
    if (t === 'string' || t === 'number' || t === 'boolean') {
        ...
        for (var _i2 in param) {
            var k = key == null ? _i2 : key + (param instanceof Array ? '[' + _i2 + ']' : '.' + _i2);
            paramStr += urlEncode(param[_i2], k, encode);
        }
    }
    return paramStr;
