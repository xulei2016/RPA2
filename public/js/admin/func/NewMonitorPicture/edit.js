$(function () {
    let url_prefix = "/admin/rpa_new_monitor_picture/";
    ...
    function bindEvent() {
        ...
        //定义时间按钮事件
        let st = '#start_at_final';
        let et = '#end_at_final';

        laydate.render({ elem: st, type: 'date'});
        ...
        $(document).ready(function () {
            var generalAddress = $('#general-address');
            var deepAddress = $('#deep-address');
            var res = getHighLightDifferent(generalAddress.attr('val'), deepAddress.attr('val'));
            generalAddress.html(res[0]);
            deepAddress.html(res[1]);
        });
    ...
    function add(e) {
        ...
        var start_at = $('#start_at_final');
        var start_at_final = start_at.val();
        var end_at = $('#end_at_final');
        var end_at_final = end_at.val();
        ...
        if (start_at_final !== start_at.attr('item')) {
            flag = false;
        }
        if (end_at_final !== end_at.attr('item')) {
            flag = false;
        }
        if (!flag) { // 发生修改
            Swal.fire({
