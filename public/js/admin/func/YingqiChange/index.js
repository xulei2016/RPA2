$(function(){
    ...
    function bindEvent(){
        ...
        $("#pjax-container section.content #toolbar #exportAll").on('click', function(){
            ...
            //location.href="/admin/rpa_yq_change/export?"+$url;
            window.open("/admin/rpa_yq_change/export?"+$url);
        });

        //导出选中
        $("#pjax-container section.content #toolbar #export").on('click', function(){
            var ids = RPA.getIdSelections('#tb_departments');
            //location.href="/admin/rpa_yq_change/export?id="+ids;
            window.open("/admin/rpa_yq_change/export?id="+ids);
        });
    ...
    function getSearchGroup(){
        ...
        var temp = {
            ...
            order_num : $("#pjax-container #search-group #orderNum").val()
            ...
            columns: [{
                ...
                }, 
                {
                    ...
                    formatter: function(value, row, index){
                        var result = "";
                        if(value){
                            result = value.replace("1",'新增');
                            result = result.replace("2",'<span style="color:#eea236">变更</span>');
                ...
                }, 
                 {
                    ...
                    formatter: function(value, row, index){
                        let res = "";
                        if(row.old_account){
                            res = "<div>"+row.old_bank_name+"<br/>"+row.old_account+"</div>";
                        }
                ...
                },
                {
                    ...
                    formatter: function(value, row, index){
                        let res = "";
                        res = "<div>"+row.opening_bank+"<br/>"+row.bank_card_num+"</div>";
                ...
                }, 
                {
                    ...
                    formatter: function(value, row, index){
                        ...
                        if(status == 1){
                            result += " <a href='javascript:;' class='btn btn-sm btn-success' onclick=\"operation($(this));\" url='/admin/rpa_yq_change/"+id+"' title='办理结果'>办理结果</a>";
                        }
                        if(status == 3){
                            result += " <a href='javascript:;' class='btn btn-sm btn-primary' onclick=\"operation($(this));\" url='/admin/rpa_yq_change/"+id+"' title='办理'>审核</a>";
                        }
