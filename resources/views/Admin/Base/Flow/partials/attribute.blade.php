<ul class="nav nav-tabs" id="attributeTab">
    <li class="active">
        <a href="#attrBasic">常规</a>
    </li>
    <li>
        <a href="#attrForm">表单</a>
    </li>
    <li>
    ...
    <li>
        <a href="#attrStyle">样式</a>
    </li>
</ul>

<form class="form-horizontal" method="post" id="flow_attribute" name="flow_attribute" action="{{route('node.update',['id'=>$node->id])}}">
    <div class="tab-content">
        <div class="tab-pane active" id="attrBasic">

            <div class="control-group">
                <label class="control-label" for="node_title">步骤名称</label>
                <div class="controls">
                    <input type="text" id="node_title" name="node_title" placeholder="步骤名称" value="{{$node->node_title}}">
                </div>
            </div>
            ...
            <div id="current_flow" @if($node->position==2) class="hide" @endif>
                <div class="offset1">
                    <select multiple="multiple" size="6" name="node_to[]" id="node_multiple">
                        @foreach($next_node as $v) 
                            @if($v->next_node)
                                <option value="{{$v->next_node_id}}" selected="selected">{{$v->next_node->node_title}}</option>
                            @endif 
                        @endforeach @foreach($beixuan_node as $v)
                            <option value="{{$v->node_id}}">{{$v->node->node_title}}</option>
                        @endforeach
                    </select>
                </div>
            </div>

            <div id="child_flow" @if($node->position==2) class="show" @else class="hide" @endif>
                <div class="control-group">
                ...
                <div class="control-group" class="hide">
                    ...
                    <div class="controls">
                        <label class="radio inline">
                            <input type="radio" name="child_after" value="1" @if($node->child_after==1) checked @endif> 同时结束父流程
                        </label>
                        <label class="radio inline">
                            <input type="radio" name="child_after" value="2" @if($node->child_after==2) checked @endif> 返回父流程步骤
                        </label>
                    </div>
                </div>

                <div class="control-group @if($node->child_after==1) hide @endif" id="child_back_id">
                    <label class="control-label">返回步骤</label>
                    <div class="controls">
                        <select name="child_back_node">
                            <option value="0">--默认--</option>
                            @foreach($nodes as $v)
                            <option value="{{$v->id}}" @if($node->child_back_node==$v->id) selected="selected" @endif >{{$v->node_title}}</option>
                            @endforeach
                        </select>
            ...
            <div class="control-group">
                ...
                <div id="auto_person_4" class="hide">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" placeholder="指定主办人" name="auto_sponsor_uids">
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" placeholder="指定经办人" name="auto_respon_uids">
                            <a href="javascript:void(0);">选择</a>
                        </div>
                    </div>
                </div>
                <div id="auto_person_5" class="hide">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" placeholder="指定角色" name="auto_role_ids">
                            <a href="javascript:void(0);">选择</a>
                        </div>
                    </div>
                </div>
            ...
            <div class="control-group">
                <div class="controls">
                    <label class="checkbox">
                        <input type="checkbox" name="agent_pass" value="is_node">经办人可以转交下一步
                    </label>
                </div>
            </div>
        ...
        <div class="tab-pane" id="attrStyle">
            ...
            <div class="control-group">
                <label class="control-label" for="node_title">尺寸</label>
                <div class="controls">
                    <input type="text" class="input-small" name="style_width" id="style_width" placeholder="宽度PX" value="{{$node->style_width}}"> X
                    <input type="text" class="input-small" name="style_height" id="style_height" placeholder="高度PX" value="{{$node->style_height}}">
                </div>
            </div>

            <div class="control-group">
                <label class="control-label" for="node_title">字体颜色</label>
                <div class="controls">
                    <input type="text" class="input-small" name="style_color" id="style_color" placeholder="#000000" value="{{$node->style_color}}">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <div class="colors" org-bind="style_icon">
                        <ul>
                            <li class="Black active" org-data="icon-star-empty" title="Black">
                                <i class="icon-star-empty icon-white"></i>
                            </li>
                            <li class="red" org-data="icon-ok" title="Red">
                                <i class="icon-ok icon-white"></i>
                            </li>
                            <li class="green" org-data="icon-remove" title="Green">
                                <i class="icon-remove icon-white"></i>
                            </li>
                            <li class="blue" org-data="icon-refresh" title="Blue">
                                <i class="icon-refresh icon-white"></i>
                            </li>
                            <li class="aero" org-data="icon-plane" title="Aero">
                                <i class="icon-plane icon-white"></i>
                            </li>
                            <li class="grey" org-data="icon-play" title="Grey">
                                <i class="icon-play icon-white"></i>
                            </li>
                            <li class="orange" org-data="icon-heart" title="Orange">
                                <i class="icon-heart icon-white"></i>
                            </li>
                            <li class="yellow" org-data="icon-random" title="Yellow">
                                <i class="icon-random icon-white"></i>
                            </li>
                            <li class="pink" org-data="icon-home" title="Pink">
                                <i class="icon-home icon-white"></i>
                            </li>
                            <li class="purple" org-data="icon-lock" title="Purple">
...
<script type="text/javascript">
            ...
            if ($("#node_multiple option:selected").length > 1) {
                $("#node_multiple option").each(function () {
                    if ($(this).val() > 0 && $(this).attr("selected")) {
                        ...
                        $('#ctbody').append(s);
                        ids += id + ',';
                        //flow_id 是流程设计的ID，  node_id 是步骤ID
                        if (get_con_url) {
                            //var get_con_url = "/index.php?s=/index/get_con/flow_id/62/node_id/"+id+".html";
                            $.post(get_con_url, { "flow_id": flow_id, "node_id": node_id, 'next_node_id': id }, function (data) {
                                $.each(data, function (i, n) {
                                    if (i == id && _id('conList_' + i)) {
                                        $('#conList_' + i).append(n.option);
                                        $('#node_in_desc_' + i).val(n.desc);
                                    }
                                })
                            }, 'json');
                        }
    ...
    $(function () {
        ...
        //步骤select 2
        $('#node_multiple').multiselect2side({
        ...
        $("#flow_attribute").submit(function () {
            //条件检测
            var cond_data = $("#node_condition").val();
            if (cond_data !== '') {
                ...
                for (var i = 0; i < pcarr.length; i++) {
                    if (pcarr[i] !== '') {
                        ...
                        if (obj.length > 0) {
                            ...
                            _id('node_in_set_' + pcarr[i]).value = constr;
                        } else {
                            _id('node_in_set_' + pcarr[i]).value = '';
                        }
                    }
                }
            }
