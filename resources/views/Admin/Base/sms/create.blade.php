@component('admin.widgets.addForm')
    @slot('formContent')
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <input type="text" class="form-control" name="unique_name" id="unique_name" placeholder="名称">
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                ...
                <div class="available_list">
                    @foreach($gateways as $gateway)
                        <label><input type="checkbox" value="{{ $gateway->unique_name }}">{{ $gateway->name }}</label>
                    @endforeach
                </div>
                <input type="hidden" id="available_list" name="available_list">
            </div>
        </div>
        <div class="form-group row">
    ...
    @endslot
    @slot('formScript')
        <script src="{{URL::asset('/js/admin/base/sms/add.js')}}"></script>
    @endslot
@endcomponent