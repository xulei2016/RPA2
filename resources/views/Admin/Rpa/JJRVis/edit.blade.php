@component('admin.widgets.addForm')    
    ...
    <div class="form-group">
        ...
        <div class="form-group ">
            ...
            <div class="col-sm-10">
                @foreach($admin as $v)
                    <label><input type="checkbox" class="select-single" name="jsondata[]" value="{{$v->realName}}" @if(in_Array($v->realName,$info->data))checked @endif>{{$v->realName}}</label>
                @endforeach
            </div>
        </div>
        <div class="form-group">
    ...
    @slot('formScript')
        <script src="{{URL::asset('/js/admin/rpa/JJRVis/edit.js')}}"></script>
    @endslot
@endcomponent